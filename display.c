...
#define EEPROM_NUMBER (uint8_t)100           /* Här sparar vi talet på displayerna, uppdateras vid byte */
#define EEPROM_OUTPUT_ENABLED 101   /* Här sparas om displayerna är på */
#define EEPROM_COUNT_ENABLED 102    /* Här sparas om uppräkning är på */
#define EEPROM_COUNT_DIRECTION 103  /* Här sparas uppräkningsriktning */
#define EEPROM_INITIALIZED 104      /* Indikerar om värden har sparats i EEPROM (0 = true) */
...
static inline void display_eeprom_check(void);
...
/********************************************************************************
* display_init: Initierar hårdvara för 7-segmentsdisplayer. Dessutom initiseras
                timer_digit så att denna löper ut efter 1 ms för att skifta
				aktiv display. Slutligen initieras timer_count_speed så att 
				uppräkning sker som defult med 1 sek.
********************************************************************************/
void display_init(void)
{
	...
	display_eeprom_check();
	return;
}

/********************************************************************************
* display_reset: Återställer 7-segmentsdisplayer till startläget.
                 Först återställs timerkretsarna, sedan stängs displayerna av
				 sedan nollställs variablerna och räkningshållet ställs för att 
				 räkna up och current digit sätts till display 1
********************************************************************************/
...
*                         Vid påslagna displayer returneras true, annars false.
                          I praktiken kollar vi om avbrott är aktiverat på 
						  timer_digit och returnerar detta isåfall.
********************************************************************************/
...
*                        är aktiverat returneras true, annars false.
                         I praktiken kontrolleras om avbrott är aktiverat på 
						 timer_count_speed.
********************************************************************************/
...
/********************************************************************************
* display_enable_output: Sätter på 7-segmentsdisplayer samt skriver till
                         eeprom-minnet att displayerna är på.
********************************************************************************/
...
/********************************************************************************
* display_disable_output: Stänger av 7-segmentsdisplayer. Först återställs
                          timer_digit, vilket innbär att den både stängs av samt
						  att dess räknare nollställs. Samt skriver till
						  eeprom-minnet att displayerna är av.
********************************************************************************/
...
********************************************************************************/
int display_set_number(uint8_t new_number)
{
...
********************************************************************************/
int display_set_radix(uint8_t new_radix)
{
	if(new_radix == 2)
	{
		max_val = 3;
...
* display_count: Räknar upp eller ned tal på 7-segmentsdisplayer.
				 ...
				 2. Om timer_count_speed löper ut, räkna upp eller ned number
				    beroende på uppräkningshastighet, annars görs ingeting. 
					
				 3. Om uppräningshastigheten är uppåt, räkna upp till number
				    till och med max_val. Om number > max_val, nollställ number.
					
				 4. Om uppräkningsriktningen är nedåt, räkna ner number till 0.
				    Om number redan är 0. Sätt number = max_val.
					
				 5. Skriver nummret till eeprom-minnet
				 
				 6. Uppdatera tiotalet och entalet.
********************************************************************************/
void display_count(void)
{
	...
	if (timer_elapsed(&timer_count_speed))
	{
		...
		eeprom_write_byte(EEPROM_NUMBER, number); /* Sparar det nya värdet i EEPROM */
	}
...
********************************************************************************/
void display_set_count_direction(enum display_count_direction new_direction)
{
...
*                                 ut på 7-segmentsdisplayer. Samt skriver vilken
                                  riktning som displayerna räknar i till eeprom.
********************************************************************************/
void display_toggle_count_direction(void)
{
	...
	if (count_direction == DISPLAY_COUNT_DIRECTION_UP)
	{
		eeprom_write_byte(EEPROM_COUNT_DIRECTION, DISPLAY_COUNT_DIRECTION_UP);
	}
	else
	{
		eeprom_write_byte(EEPROM_COUNT_DIRECTION, DISPLAY_COUNT_DIRECTION_DOWN);
	}
	return;
}
...
********************************************************************************/
void display_set_count(enum display_count_direction direction,
                       uint16_t count_speed_ms)
{
...
*                       (via anrop av funktionen display_set_count). Samt skriver 
                        till eeprom-minnet om räkning är på.
********************************************************************************/
...
*                        på 7-segmentsdisplayerna. Samt skriver till eeprom 
                         om räkning är av.
********************************************************************************/
...
* display_update_output: Skriver ny siffra till aktiverad 7-segmentsdisplay.
						 ...
						 -digit: Siffran som skalll skrivas ut.
********************************************************************************/
static inline void display_update_output(const uint8_t digit)
{
...
/************************************************************************/
/* display_eeprom_check: Kollar ifall vi har sparat värden i EEPROM och
                         läser i så fall in den.
						 
						 1. Vi läser av adressen EEPROM_INITIALIZED för 
						    att kolla om något är sparat. Om vi har gjort
							det. Så läses värderna in.
							
						 2. Vi läser in det gamla talet från EEPROM_NUMBER
						    och sparar i variablen number.
							
						 3. Om displayerna var på (EEPROM_COUNT_ENABLED = 1)
						    senast sätter vi på displayerna.
							
						 4. Om uppräkning var på (EEPROM_COUNT_ENABLED == 1)
						    innan vi stängde av sätter vi på uppräkning.  
						 
						 5. Om inga värden låg i EEPROM så initierar vi 
						    adresserna.                                 */
/************************************************************************/
static inline void display_eeprom_check(void)
{
